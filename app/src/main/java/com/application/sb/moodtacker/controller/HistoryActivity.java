...
public class HistoryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (MainActivity.moodsArrayList.size() > 0) {  // Si le tableau Mood comprend au moins 1 element
            int i = 0;
            // Number of history
            int historyNb = 7;
            for(int j = MainActivity.moodsArrayList.size()-1 ; j > (MainActivity.moodsArrayList.size() - 1) - historyNb; j--){

                if(j >= 0){
                // New Relative Layout
                RelativeLayout relativeLayout = new RelativeLayout(this);
                RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(sizeTab[MainActivity.moodsArrayList.get(j).getPosition()], height / historyNb);
                ...
                textView.setBackgroundColor(getResources().getColor(Moods.colorsTab[MainActivity.moodsArrayList.get(j).getPosition()]));
                relativeLayout.addView(textView);

                //New imageButton for the relative layout
                if (MainActivity.moodsArrayList.get(j).getComment().length() >= 1){
                    ...
                    imageButton.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            Toast.makeText(HistoryActivity.this, MainActivity.moodsArrayList.get(finalX).getComment(), Toast.LENGTH_SHORT).show();
                        }
                    });
